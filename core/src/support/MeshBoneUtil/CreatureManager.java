 ...
 // Update the region switching properties
    private void UpdateRegionSwitches(String animation_name_in)
    {
    	if (animations.containsKey(animation_name_in)) {
    		...
    		support.MeshBoneUtil.MeshDisplacementCacheManager displacement_cache_manager = cur_animation.displacement_cache;
    		Vector<MeshDisplacementCache> displacement_table =
    			displacement_cache_manager.displacement_cache_table.get(0);

    		support.MeshBoneUtil.MeshUVWarpCacheManager uv_warp_cache_manager = cur_animation.uv_warp_cache;
    		Vector<MeshUVWarpCache> uv_swap_table =
    			uv_warp_cache_manager.uv_cache_table.get(0);

    		support.MeshBoneUtil.MeshRenderBoneComposition render_composition =
    		...
    		{
    			support.MeshBoneUtil.MeshRenderRegion cur_region = all_regions.get(i);
...
*/
    ...
    private void UpdateRegionColours()
    {
    	support.MeshBoneUtil.MeshRenderBoneComposition render_composition =
    			target_creature.render_composition;
		Vector<MeshRenderRegion> cur_regions = render_composition.getRegions();

		for (int i = 0; i < cur_regions.size(); i++) {
			support.MeshBoneUtil.MeshRenderRegion cur_region = cur_regions.get(i);
            ...
            float input_run_time)
	{
		...
		support.MeshBoneUtil.MeshUVWarpCacheManager uv_warp_cache_manager = cur_animation.uv_warp_cache;
		support.MeshBoneUtil.MeshOpacityCacheManager opacity_cache_manager = cur_animation.opacity_cache;
		...
		for(int j = 0; j < cur_regions.size(); j++) {
			support.MeshBoneUtil.MeshRenderRegion cur_region = cur_regions.get(j);
            ...
            float input_run_time)
    {
    	...
    	support.MeshBoneUtil.MeshBoneCacheManager bone_cache_manager = cur_animation.bones_cache;
	...
	public void AlterBonesByAnchor(HashMap<String, MeshBone> bones_map, String animation_name_in)
	{
